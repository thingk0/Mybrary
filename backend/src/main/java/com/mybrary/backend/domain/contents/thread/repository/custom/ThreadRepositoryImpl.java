...
public class ThreadRepositoryImpl implements ThreadRepositoryCustom {
      ...
      public Optional<List<GetThreadDto>> getRandomThreadDtoResults(Long memberId, Pageable pageable) {
            ...
            return Optional.ofNullable(query.select(Projections.constructor(GetThreadDto.class, thread.id, thread.createdAt, member.id, member.name, member.nickname, image.url, image.id, thread.isPaperPublic, thread.isScrapEnabled))
                                            ...
                                            .leftJoin(member).on(mybrary.member.id.eq(member.id))
                                            .leftJoin(image).on(member.profileImage.id.eq(image.id))
                                            .where(member.id.notIn(query.select(followSub.following.id).from(followSub).where(followSub.follower.id.eq(memberId))).and(member.id.ne(memberId)).and(member.isProfilePublic.eq(true)))
                                            .groupBy(thread.id)
                                            .orderBy(thread.createdAt.desc())
                                            .offset(pageable.getOffset())
                                            .limit(pageable.getPageSize())
