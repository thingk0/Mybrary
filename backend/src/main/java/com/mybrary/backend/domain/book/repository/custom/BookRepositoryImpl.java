...
public class BookRepositoryImpl implements BookRepositoryCustom {
    ...
    public List<BookGetDto> getAllBookByCategoryId(Long categoryId) {
        ...
        return query.select(
                    ...
                    .from(book)
                    .innerJoin(image1).on(book.coverImage.id.eq(image1.id))
                    .leftJoin(bookMarker).on(bookMarker.book.id.eq(book.id))
                    .innerJoin(pickBook).on(pickBook.book.id.eq(book.id))
                    .innerJoin(member).on(book.member.id.eq(member.id))
                    .innerJoin(image2).on(member.profileImage.id.eq(image2.id))
